...
class CacheClient:
    ...
    def simulate_searches(self, n_searches=100):
        ...
        #generar lista que contiene el numero de keys que buscar
        keys_to_search = [f"{i}" for i in np.random.randint(1, 101, n_searches)]

         # Métricas
        time_without_cache = 0 # Registra el tiempo estimado de búsqueda sin cache.
        time_with_cache = 0 #Registra el tiempo real de búsqueda con cache.
        avoided_json_lookups = 0 #Cuenta cuantas veces se evito una busqueda en el JSON

        count = 0
        tiempo_total = 0  # Agregar una variable para rastrear el tiempo total de simulación
        ...
        for key in keys_to_search:
            ...
            tiempo_total += elapsed_time  # Agregar el tiempo de esta búsqueda al tiempo total
        ...
        print(f"Tiempo total de simulacion: {tiempo_total:.2f} seconds")# seccion agregada para mostrar el tiempo total que tarda todo el proceso
